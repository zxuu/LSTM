...
net = nn.LSTM(input_size=in_dim, hidden_size=hidden_dim, num_layers=3, bidirectional=True)
h0, c0 = torch.randn(num_layers*bidirectional, batch_size, hidden_dim), torch.randn(num_layers*bidirectional, batch_size, hidden_dim)
...
for i in range(epoch):
    ...
    for texts_idd, labels_idd in loader:    # 一句话
        ...
        optimizer.zero_grad()
        # output:输出每个time_step的结果，所以是[seq_len, batch, hidden_dim]
        output, (h_n, c_n) = net(input, (h0, c0))    # output:[seq_len, batch, hidden_dim] h_n:[num_layers*bidirectional, batch, hidden_dim] c_n:[num_layers*bidirectional, batch, hidden_dim]
        loss = 0
        is_right = [0]    # 记录一个句子的准确度
        for word_output, label in zip(output, labels_y):
            loss = loss + criterion(word_output, label)  # 要把每个词的loss累加
            _, idx = word_output.max(dim=1)
            # 记录预测是否正确
            # if label.item()!=26:
